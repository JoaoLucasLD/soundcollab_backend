...
@Injectable()
export class InstrumentMatchStrategy implements MatchmakingStrategy {
  readonly key = 'instrument' as const;

  calculate({ requester, candidate, filters }: MatchmakingStrategyInput): number {
    if (filters.instrument) {
      return candidateHasValue(candidate.instruments, filters.instrument) ? 1 : 0;
    }

    return calculateJaccardSimilarity(
      requester.instruments,
